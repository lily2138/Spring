...
from emp
group by deptno
having min(sal) != max(sal) 		-- min(sal) <> max(sal)
order by deptno;
...
and deptno in (10,20,30) and comm is not null 
and sal between 1500 and 3000
order by hiredate, sal desc;         -- 이중 order by / hiredate 은 오름차순 sal 은 내림차순 
...
group by deptno
having 평균급여 >=2000
order by 평균급여 desc; 		-- order by 2 desc;
